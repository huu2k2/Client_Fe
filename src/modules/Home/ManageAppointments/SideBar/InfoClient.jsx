...
const InfoClient = ({register, getInfo, setValue, isSidebarOpen }) => {
  return (
    <div className="w-fit h-fit  flex-col justify-start items-start gap-5 inline-flex  pl-4 py-5">
      <div className="text-rose-800 text-lg font-medium leading-7">
        Thông tin khách hàng
      </div>
      {muiltyRow.map((row) => (
     <RowComponent register={register} getInfo={getInfo} isSidebarOpen={isSidebarOpen} setValue={setValue}  name={row.name} key={row.id} title={row.title} type={row.type} placeholder={row.placeholder} unit={row.unit} />
      ))}
    </div>
  );
};
